...
if [ ! -d $TEST_ROOT ];then
    echo "Not found the directory --> ${TEST_ROOT} !"
    exit 1
fi
...
echo "Start to create test lmdb!"
$TOOLS_ROOT/convert_imageset --shuffle=true --encoded=true \
--dim_label=1 --resize_height=256 --resize_width=256 --check_size=false \
$IMAGE_ROOT/test/ $IMAGE_LIST_ROOT/test.txt  $LMDB_TEST
echo "End to create test lmdb!"

echo "Start to create val lmdb!"
$TOOLS_ROOT/convert_imageset --shuffle=true --encoded=true \
--dim_label=1 --resize_height=256 --resize_width=256 --check_size=false \
$IMAGE_ROOT/val/ $IMAGE_LIST_ROOT/val.txt  $LMDB_VAL
echo "End to create val lmdb!"
